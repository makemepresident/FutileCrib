...
class GameHandler:
    ...
    def peggingRound(self):
        ...
        while not self.players[0].handIsEmpty() and not self.players[1].handIsEmpty(): # Either player has cards
            for i in range(len(self.players)):
                ...
                self.addHandMemory(played_card, played)
                if played == 0:
                    self.peg_count = 0
                    self.players[(player_index + 1) % 2].score += 1
                    continue
                self.players[(player_index) % 2].score += p.checkRun(played_card)
                if played.getValue() + self.peg_count == 31 or \
                played.getValue() + self.peg_count == 15:
    ...
    def addHandMemory(self, played_card, card):
        if card == 0:
            played_card = []
            return
    ...
    def cribCall(self):
        ...
        for i in range(len(self.players)):
            for j in range(3):
                if j == 0 or j == 1:
                    print(self.players[i].name + "'s hand:\n", self.players[i].getHand())
                    print()
                if j == 0:
...
class Points:
    ...
    def checkRun(self, hand):
        ...
        powerset = list(self.powerset(hand))
        if hand in powerset:
            return len(hand)
        elif hand[:2] in powerset or hand[1:] in powerset:
            return 3
        else:
            return 0
